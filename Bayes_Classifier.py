...
for class_value in range(9):
    ...
    plt.scatter(X_test[row_ix, 0], X_test[row_ix, 1],X_test[row_ix, 2],X_test[row_ix, 3],X_test[row_ix, 4],X_test[row_ix, 5],X_test[row_ix, 6],X_test[row_ix, 7],X_test[row_ix, 8],X_test[row_ix, 9],label=f' Class {class_value}')
    plt.legend()
plt.xlabel('X_test')
plt.ylabel('Class Value')
plt.title('Decision Boundary with Bayesian Classifier (Generated data)')
#     #plt.show()
# # show the plot
plt.show()
